...
class Transport extends EnhancedEventEmitter {
  ...
  Transport({
    ...
    _logger.debug('constructor(b) [id:$id, direction:${direction.value}]');
  ...
  void _handleHandler() {
    ...
    handler.on(
      ...
      (Map data) {
        ...
        _logger.debug('_handleHandler should now emit');
    ...
    handler.on(
      ...
      (Map data) {
        _logger.debug('snow connectionstatechange');

        _logger.debug('@connectionstatechange');
  ...
  Future<void> close() async {
    ...
    _logger.debug('close() Close the Transport');
  ...
  Future<void> _produce(ProduceArguments arguments) async {
    try {
      ...
      try {
        ...
        debugPrint("validateRtpParameters Passed");
        ...
        debugPrint("validateRtpParameters Passed seconds");


        Producer producer = Producer(
          id: id??"49894t594-9340343",
        ...
        debugPrint("validateRtpParameters Passed third");
      ...
      debugPrint("Here is the wifbf ${error.toString()}");
  ...
  Future<void> _consume(ConsumeArguments arguments) async {
    ...
    if (!canConsume) {
      ...
      _logger.debug('can consume this Producer');

    }
    try {
      _logger.debug('xxcscn er ${arguments.rtpParameters.toMap()}');
      ...
      _logger.debug('_handler.receive completed ${arguments.rtpParameters}');
